...
if [ -z "${BASH_VERSINFO:-}" ] || [ "${BASH_VERSINFO[0]}" -lt 4 ]; then
    ...
    echo "[!] No bash >= 4.0 found (associative arrays required). Install one via: brew install bash" >&2
    exit 1
fi

set -u

COMPILER_TAG="macOS-GCC"
GCC_VERSION_FALLBACK=$(brew list --versions gcc 2>/dev/null | awk '{print $2}' | cut -d. -f1)
CXX_COMPILER="${CXX_BIN:-$(brew --prefix)/bin/g++-${GCC_VERSION_FALLBACK}}"
C_COMPILER="${C_BIN:-$(brew --prefix)/bin/gcc-${GCC_VERSION_FALLBACK}}"
...
echo "Using CXX: ${CXX_COMPILER}"
echo "Using CC:  ${C_COMPILER}"
