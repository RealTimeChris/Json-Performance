...
template<> struct jsonifier::core<search_metadata_data> {
	...
	static constexpr auto parseValue = createValueReflected<&value_type::completed_in, &value_type::max_id, &value_type::max_id_str, &value_type::next_results, &value_type::query,
...
template<> struct jsonifier::core<hashtag> {
	...
	static constexpr auto parseValue = createValueReflected<&value_type::text, &value_type::indices>();
};

template<> struct jsonifier::core<large_data> {
	using value_type				 = large_data;
	static constexpr auto parseValue = createValueReflected<&value_type::w, &value_type::h, &value_type::resize>();
};

template<> struct jsonifier::core<sizes_data> {
	using value_type				 = sizes_data;
	static constexpr auto parseValue = createValueReflected<&value_type::medium, &value_type::small, &value_type::thumb, &value_type::large>();
};

template<> struct jsonifier::core<media_data> {
	using value_type				 = media_data;
	static constexpr auto parseValue = createValueReflected<&value_type::id, &value_type::id_str, &value_type::indices, &value_type::media_url, &value_type::media_url_https,
...
template<> struct jsonifier::core<url_data> {
	...
	static constexpr auto parseValue = createValueReflected<&value_type::expanded_url, &value_type::display_url, &value_type::indices>();
};

template<> struct jsonifier::core<user_mention> {
	using value_type				 = user_mention;
	static constexpr auto parseValue = createValueReflected<&value_type::screen_name, &value_type::name, &value_type::id, &value_type::id_str, &value_type::indices>();
};

template<> struct jsonifier::core<status_entities> {
	using value_type				 = status_entities;
	static constexpr auto parseValue = createValueReflected<&value_type::hashtags, &value_type::symbols, &value_type::urls, &value_type::user_mentions, &value_type::media>();
};

template<> struct jsonifier::core<metadata_data> {
	using value_type				 = metadata_data;
	static constexpr auto parseValue = createValueReflected<&value_type::result_type, &value_type::iso_language_code>();
};

template<> struct jsonifier::core<description_data> {
	using value_type				 = description_data;
	static constexpr auto parseValue = createValueReflected<&value_type::urls>();
};

template<> struct jsonifier::core<user_entities> {
	using value_type				 = user_entities;
	static constexpr auto parseValue = createValueReflected<&value_type::description>();
};

template<> struct jsonifier::core<user_data_twitter> {
	using value_type				 = user_data_twitter;
	static constexpr auto parseValue = createValueReflected<&value_type::id, &value_type::id_str, &value_type::name, &value_type::screen_name, &value_type::location,
...
template<> struct jsonifier::core<status> {
	...
	static constexpr auto parseValue = createValueReflected<&value_type::metadata, &value_type::created_at, &value_type::id, &value_type::id_str, &value_type::text,
...
template<> struct jsonifier::core<twitter_message> {
	...
	static constexpr auto parseValue = createValueReflected<&value_type::statuses, &value_type::metadata>();
};
...
namespace glz {
	...
	template<> struct meta<client_status> {
		using value_type			= client_status;
	...
	template<> struct meta<tags> {
		using value_type			= tags;
...
namespace jsonifier {
	...
	template<> struct core<available_tag> {
		using value_type				 = available_tag;
		static constexpr auto parseValue = createValueReflected<&value_type::name, &value_type::moderated, &value_type::id>();
	};

	template<> struct core<default_reaction_emoji_data> {
		using value_type				 = default_reaction_emoji_data;
		static constexpr auto parseValue = createValueReflected<>();
	};

	template<> struct core<icon_emoji_data> {
		using value_type				 = icon_emoji_data;
		static constexpr auto parseValue = createValueReflected<&value_type::name, &value_type::id>();
	};

	template<> struct core<permission_overwrite> {
		using value_type				 = permission_overwrite;
		static constexpr auto parseValue = createValueReflected<&value_type::type, &value_type::id, &value_type::deny, &value_type::allow>();
	};

	template<> struct core<channel_data> {
		using value_type = channel_data;
		static constexpr auto parseValue =
			createValueReflected<&value_type::version, &value_type::type, &value_type::position, &value_type::permission_overwrites, &value_type::name, &value_type::id,
	...
	template<> struct core<emoji_element> {
		using value_type				 = emoji_element;
		static constexpr auto parseValue = createValueReflected<&value_type::version, &value_type::roles, &value_type::require_colons, &value_type::name, &value_type::managed,
			&value_type::id, &value_type::available, &value_type::animated>();
	};

	template<> struct core<avatar_decoration_data_data> {
		using value_type				 = avatar_decoration_data_data;
		static constexpr auto parseValue = createValueReflected<&value_type::sku_id, &value_type::asset>();
	};
...
namespace jsonifier {
	...
	template<> struct core<member_user> {
		using value_type				 = member_user;
		static constexpr auto parseValue = createValueReflected<&value_type::username, &value_type::public_flags, &value_type::id, &value_type::discriminator, &value_type::bot,
			&value_type::avatar_decoration_data>();
	};

	template<> struct core<guild_member_data> {
		using value_type				 = guild_member_data;
		static constexpr auto parseValue = createValueReflected<&value_type::user, &value_type::roles, &value_type::pending, &value_type::mute, &value_type::joined_at,
			&value_type::flags, &value_type::deaf, &value_type::avatar>();
	};

	template<> struct core<activity_emoji> {
		using value_type				 = activity_emoji;
		static constexpr auto parseValue = createValueReflected<&value_type::name, &value_type::id, &value_type::animated>();
	};

	template<> struct core<activity> {
		using value_type = activity;
		static constexpr auto parseValue =
			createValueReflected<&value_type::type, &value_type::state, &value_type::name, &value_type::id, &value_type::emoji, &value_type::created_at, &value_type::url>();
	};

	template<> struct core<client_status> {
		using value_type				 = client_status;
		static constexpr auto parseValue = createValueReflected<&value_type::web, &value_type::desktop, &value_type::mobile>();
	};

	template<> struct core<presence_user> {
		using value_type				 = presence_user;
		static constexpr auto parseValue = createValueReflected<&value_type::id>();
	};

	template<> struct core<presence> {
		using value_type				 = presence;
		static constexpr auto parseValue = createValueReflected<&value_type::user, &value_type::client_status, &value_type::activities>();
	};

	template<> struct core<tags> {
		using value_type				 = tags;
		static constexpr auto parseValue = createValueReflected<&value_type::bot_id>();
	};

	// Continue with the same pattern for the remaining structures...

}// namespace jsonifier
namespace jsonifier {

	template<> struct core<role_data> {
		using value_type				 = role_data;
		static constexpr auto parseValue = createValueReflected<&value_type::version, &value_type::tags, &value_type::position, &value_type::permissions, &value_type::name,
			&value_type::mentionable, &value_type::managed, &value_type::id, &value_type::hoist, &value_type::flags, &value_type::color>();
	};

	template<> struct core<soundboard_sound> {
		using value_type = soundboard_sound;
		static constexpr auto parseValue =
			createValueReflected<&value_type::volume, &value_type::user_id, &value_type::sound_id, &value_type::name, &value_type::guild_id, &value_type::available>();
	};

	template<> struct core<sticker> {
		using value_type				 = sticker;
		static constexpr auto parseValue = createValueReflected<&value_type::version, &value_type::type, &value_type::tags, &value_type::name, &value_type::id,
			&value_type::guild_id, &value_type::format_type, &value_type::available>();
	};

	template<> struct core<guild_data> {
		using value_type				 = guild_data;
		static constexpr auto parseValue = createValueReflected<&value_type::lazy, &value_type::system_channel_flags, &value_type::members, &value_type::joined_at,
			...
			&value_type::member_count, &value_type::embedded_activities, &value_type::premium_tier, &value_type::application_command_counts, &value_type::stage_instances,
			&value_type::version, &value_type::owner_id, &value_type::emojis>();
	};

	template<> struct core<discord_message> {
		using value_type				 = discord_message;
		static constexpr auto parseValue = createValueReflected<&value_type::t, &value_type::s, &value_type::op, &value_type::d>();
	};
...
class testClass {
	...
	testClass operator[](int32_t newValue) {
